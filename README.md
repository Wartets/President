...
* Pipeline automatique complet de bout en bout (entraînement, évaluation, graphiques, rapport de synthèse), sans intervention humaine, reprenant automatiquement où il s'était arrêté après toute interruption, y compris une interruption brutale au milieu d'une combinaison individuelle (`research.run_pipeline`, manifeste sauvegardé après chaque unité de travail). Par défaut, la grille de règles couverte (`--rule-presets auto`) balaie l'intégralité des paramètres de `GameConfig` : chaque champ booléen basculé individuellement dans les deux sens, chaque mode de VP, chaque sémantique de passe, plusieurs rangs magiques et de saut de tour, plusieurs nombres de paquets forcés et de joueurs ouvreurs, les deux types de pénalité de sortie avec plusieurs tailles de reprise de cartes, chaque rôle ciblé par l'attribution stricte du reste, l'ensemble des paires de règles avancées structurantes croisées deux à deux (interactions à deux facteurs), ainsi qu'un large ensemble de combinaisons aléatoires reproductibles, croisée avec l'intégralité des profils heuristiques (hors `mcts_bot`) et des nombres de joueurs de 3 à 10. L'entraînement des agents linéaire et neuronal ne rallonge jamais sa durée totale par lancement : chaque incrément est réparti sur un curriculum de blocs consécutifs utilisant des configurations de règles (et, pour l'agent linéaire, des pools d'adversaires) tirées aléatoirement dans cette grille, afin de produire des politiques robustes à la diversité des règles plutôt qu'ajustées à une unique configuration figée. Toute exception isolée sur une combinaison individuelle (échec Ray transitoire, configuration structurellement invalide) est journalisée et n'interrompt jamais les combinaisons suivantes.
...
* **Chemins de données** : `data/`, `weights/` et `figures/` sont résolus de façon absolue par `project_paths.py`, ancrés à la racine du dépôt indépendamment du répertoire de travail courant du processus. Tout nouveau module lisant ou écrivant dans l'un de ces répertoires doit importer `project_paths` plutôt que d'utiliser une chaîne littérale `"data"`/`"weights"`/`"figures"`, afin que deux points d'entrée distincts (par exemple `research.run_pipeline` puis `research.generate_graphs` relancé séparément) opèrent toujours sur le même jeu de fichiers.
