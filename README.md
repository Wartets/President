...
* Huit profils prêts à l'emploi de complexité croissante, détaillés dans [`usage.md`, section 4.2](documentation/usage.md#42-sièges-disponibles---seats) : `random_bot`, `greedy_bot`, `rule_based_bot`, `lookahead_bot` (anticipation locale de flexibilité de main), `adaptive_bot` (filtres adaptés à la position relative), `scoring_bot` (score composite pondéré puissance/coût/rareté), `mcts_bot` (simulation Monte-Carlo par rollouts), `human_agent` (console interactive). Voir la hiérarchie de complexité complète en [`architecture.md`, section 5.3](documentation/architecture.md#53-gradation-algorithmique-des-profils-fournis).
