...
* le moteur événementiel complet (`engine.round.run_round`), qui délègue à `core.rules_engine` pour toute décision de légalité et implémente l'intégralité des règles avancées de [`rules.md`, section 6](rules.md#6-règles-supplémentaires-et-événements-de-jeu) (Putsch, Taxe Aveugle, Suites, Interception, Saut de Tour, pénalités étendues, remainder strict) ;
...
La fonction implémente une machine à états à trois phases strictement séquentielles (distribution, échange conditionnel, plis), suivies d'une clôture de manche, correspondant terme à terme à l'algorithme décrit dans [`rules.md`, section 5](rules.md#5-algorithme-dune-manche-round). Quelques décisions d'implémentation notables :
...
**Garde de sécurité `_MAX_ACTIONS_PER_TRICK`** : fixée à `max(64, n * 8)`, cette borne détecte un pli qui ne se clôture jamais (typiquement une incohérence entre `pass_type` et `skip_on_equal`, documentée comme risque connu dans [`rules.md`, section 6.3](rules.md#63-forçage-par-égalité-skip_on_equal)) et lève une `RuntimeError` explicite plutôt que de boucler indéfiniment, un choix de robustesse défensive spécifiquement motivé par le contexte d'exécution en simulation massive (`research.run_simulation`), où une manche bloquée bloquerait silencieusement tout un acteur Ray sans ce garde-fou.
...
**Ordre d'évaluation Interception avant Saut de Tour** dans `_apply_play`, reflétant explicitement la résolution [H] de la matrice de compatibilité de [`rules.md`, section 7](rules.md#7-matrice-de-compatibilité-et-résolution-des-conflits-truth-table) : le bloc d'interception est évalué et peut retourner avant que le bloc de saut de tour ne soit atteint, garantissant qu'un intercepteur dispose de la priorité sur l'application du saut.
...
### 5.2. Duplication volontaire de `_legal_options` entre agents, construction centralisée

Chaque agent automatisé (`RandomBot`, `GreedyBot`, `AggressiveBot`, `RuleBasedBot`, `LookaheadBot`, `AdaptiveBot`, `ScoringBot`, `ProbabilisticBot`, `MCTSBot`, `HumanAgent`, `RLAgent`, `TorchRLAgent`) réimplémente une méthode privée `_legal_options` de structure identique (assemblage des options uniformes et, si `straights_enabled`, des options de suite, avec réduction de la carte de Joker à sa puissance déclarée minimale). Cette duplication n'est pas accidentelle : chaque agent est conçu comme une unité autonome, sans dépendance envers les autres implémentations d'agent, de sorte qu'un agent puisse être copié, modifié ou supprimé sans effet de bord sur les autres profils. Le coût de cette duplication (douze implémentations quasi identiques) est jugé acceptable au regard de la stabilité qu'elle procure : toute modification de la logique de génération d'options reste centralisée dans `core.rules_engine.generate_uniform_plays`/`generate_sequence_plays`, la duplication ne portant que sur l'assemblage, pas sur le calcul de légalité lui-même.

La duplication ci-dessus concerne uniquement la logique interne de décision de chaque agent ; l'association nom-de-profil/classe-instanciable et la fabrique de construction (`build_agent`), en revanche, sont centralisées dans `registry.agent_registry`, consommé identiquement par `play_game.py`, `step_by_step_run.py` et `research.run_simulation`. Ce découplage évite qu'un nouveau profil doive être enregistré séparément dans chacun de ces points d'entrée.

### 5.3. Gradation algorithmique des profils fournis

Les neuf profils automatisés forment une hiérarchie de complexité croissante partageant une même famille de règles de repli, révélatrice de la conception incrémentale du projet. Le tableau correspondant, orienté utilisation plutôt que rationale, se trouve dans [`usage.md`, section 4.2](usage.md#42-sièges-disponibles---seats) :
...
* `AggressiveBot` : symétrique exact de `GreedyBot`, maximise localement la puissance résultante de la combinaison posée plutôt que de la minimiser, servant de profil de comparaison pour étudier l'effet d'une politique de dépense maximale sur le taux de victoire et la disponibilité de combinaisons de réserve en fin de manche.
...
* `LookaheadBot` : étend `RuleBasedBot` d'une couche d'anticipation locale, sans simulation des adversaires. Parmi les options de puissance résultante minimale à égalité après les mêmes filtres de préservation, retient celle qui laisse la main résiduelle la plus flexible (`_residual_flexibility`, nombre de combinaisons uniformes et suites encore jouables après retrait), bornée à `_MAX_LOOKAHEAD_CANDIDATES` options évaluées pour contenir le coût. Ce profil illustre une anticipation d'un coup de profondeur sans le coût combinatoire d'une recherche complète.
* `AdaptiveBot` : remplace le seuil fixe de préservation de `RuleBasedBot` par un seuil dynamique dépendant de la position relative de la main (`_relative_standing`, ratio entre la taille de sa main et la taille moyenne des mains adverses actives) : le filtre de réserve des grandes combinaisons n'est appliqué qu'en situation d'avance ou de parité, et est relâché en situation de retard pour tenter de rattraper le déficit. Le même indicateur module également le seuil d'invocation du Putsch et l'agressivité d'interception.
* `ScoringBot` : remplace la cascade de filtres discrets de `RuleBasedBot`/`LookaheadBot`/`AdaptiveBot` par un score composite unique et continu par option (`_composite_score`), combinant la puissance résultante normalisée, le coût de taille relatif à la main, et une pénalité de rareté décourageant de dépenser une puissance peu représentée dans la main courante. Ce profil illustre l'approche alternative, pondération d'un score composite plutôt que restriction successive de l'espace de candidats, explicitement mise en contraste avec `RuleBasedBot` ci-dessus.
* `ProbabilisticBot` : remplace le raisonnement borné à la seule main propre (`ScoringBot`, `RuleBasedBot`) par une estimation de risque fondée sur un comptage probabiliste des cartes de puissance supérieure encore susceptibles de circuler hors de la main de l'agent, dérivé de la composition totale du paquet (`core.rules_engine.num_decks`, présence de Jokers) plutôt que de la seule répartition locale des cartes détenues. Ce profil introduit une famille de raisonnement distincte des filtres déterministes et des scores composites : l'estimation probabiliste par comptage de cartes.
...
* L'ajout d'un nouvel agent au projet est désormais centralisé dans `registry.agent_registry.HEURISTIC_AGENT_REGISTRY`, consommé identiquement par `play_game.py`, `step_by_step_run.py` et `research.run_simulation` (voir [`usage.md`, section 14](usage.md#14-écrire-son-propre-agent) pour la marche à suivre complète, illustrée par `agents/scoring_bot.py` et `agents/probabilistic_bot.py`). Toute évolution future de la construction d'un agent (nouveaux paramètres de fabrique, nouveau type de profil entraînable) reste néanmoins à répercuter manuellement dans `registry.agent_registry.build_agent`.
* Le pipeline (`research.run_pipeline`) n'active son tableau de bord `ProgressManager` que pour les étapes ne gérant pas déjà leur propre affichage de progression (entraînement linéaire, balayage de taux d'apprentissage) ; les étapes qui embarquent leur propre système de rendu (`tqdm`/`LiveMonitor` dans `research.run_simulation`/`research.evaluate_agent`, tableaux `rich` périodiques dans `training.trainer.Trainer`) sont exécutées en dehors de ce contexte, deux instances `rich.live.Live` actives simultanément sur la même console produisant un affichage corrompu (barre de progression redessinée en boucle, tableaux dont seul l'en-tête reste visible).
