...
* L'ajout d'un nouvel agent au registre de `play_game.py` ou de `research.run_simulation` nécessite une modification manuelle du dictionnaire `_AGENT_REGISTRY` correspondant dans chacun de ces deux points d'entrée séparément, ces registres n'étant pas mutualisés entre les deux scripts (voir [`usage.md`, section 14](usage.md#14-écrire-son-propre-agent) pour la marche à suivre complète, illustrée par `agents/scoring_bot.py`).
