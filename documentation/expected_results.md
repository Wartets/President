...
`SYMMETRICAL`). En mode `SYMMETRICAL` avec $N$ joueurs, la moyenne empirique de chaque rang doit converger vers $VP(k) = (N-1)/2 - k$ à mesure
...
$1/5$ (persistance de rôle, due à l'avantage structurel de l'échange de cartes), sous réserve que `putsch_enabled` soit désactivé ou peu
invoqué. Chaque ligne doit sommer à 1 (loi de probabilité totale). Test : `abs(matrix.sum(axis=1) - 1.0) < 1e-9` pour chaque ligne non vide.
...
aléatoire uniforme des cartes), jamais nulle (l'inégalité résiduelle entre mains est structurelle à toute distribution finie), jamais proche
...
distribution non truquée (`strict_remainder_allocation=False`).
...
disponibles → plus de combinaisons de taille $\ge 4$ jouables). Test : coefficient de régression positif et statistiquement significatif sur
...
d'une règle désactivée (ex : aucun `SKIP_TURN` si `skip_turn_enabled=False`). Test : `set(rule_triggered_df.rule_name.unique()) ⊆
...
Attendu : sous la condition mathématique $P_{putsch}$ (main favorable), le taux de victoire du rôle `ROLE_SCUM` sollicité doit être
...
lorsque la main est déjà favorable ; l'invocation ajoute la conservation de cette main favorable en évitant l'échange désavantageux. Test :
...
joueurs sautables). Test : `max(magnitude) <= N - 1`.
